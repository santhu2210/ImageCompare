...
def send_query(access_key, secret_key, max_num_results, include_target_data, image):
    ...
    with open(image, 'rb') as f:
        imagedata = f.read()
        base64Img = imagedata.encode("base64")
    ...
    CRLF = '\r\n'
    lines = []
    lines.append("name : tarmac")
    lines.append("width : 200.0")
    lines.append("image : "+base64Img)
    lines.append("application_metadata : 496fbb6532b3863460a984de1d980bed5ebcd507")

    request_body = CRLF.join(lines)

    print "request_body ---->", request_body

    # request_body = { "name":"tarmac",
    #                  "width":32.0,
    #                  "image":base64Img
    #                 }
